...
def torch_lab2rgb(lab_image, batch_size):
    ...
    for i in range(batch_size):
        l_s = lab_image[i, 0, :, :] * 100
        a_s = (lab_image[i, 1, :, :] * 255) - 128
        b_s = (lab_image[i, 2, :, :] * 255) - 128
        ...
        temp_Y = var_Y.clone()
        mask_Y = temp_Y.pow(3.0) > 0.008856
        temp_X = var_X.clone()
        mask_X = temp_X.pow(3.0) > 0.008856
        temp_Z = var_Z.clone()
        mask_Z = temp_Z.pow(3.0) > 0.008856

        Y_1 = temp_Y.pow(3.0) * mask_Y.float()
        Y_2 = (temp_Y - 16. / 116.) / 7.787 * (~mask_Y).float()
        var_Y = Y_1 + Y_2 

        X_1 = temp_X.pow(3.0) * mask_X .float()
        X_2 = (temp_X - 16. / 116.) / 7.787 * (~mask_X).float()
        var_X = X_1 + X_2 

        Z_1 = temp_Z.pow(3.0) * mask_Z.float()
        Z_2 = (temp_Z - 16. / 116.) / 7.787 * (~mask_Z).float()
        var_Z = Z_1 + Z_2 
        ...
        temp_R = var_R.clone()
        mask_R = temp_R > 0.0031308
        R_1 = (1.055 * temp_R.pow(1/2.4) - 0.055) * mask_R.float()
        R_2 = (12.92 * temp_R) * (~mask_R).float()
        var_R = R_1 + R_2

        temp_G = var_G.clone()
        mask_G = temp_G> 0.0031308
        G_1 = (1.055 * temp_G.pow(1/2.4) - 0.055) * mask_G.float()
        G_2 = (12.92 * temp_G) * (~mask_G).float()
        var_G = G_1 + G_2
        print(var_G)
        temp_B = var_B.clone()
        mask_B = temp_B > 0.0031308
        B_1 = (1.055 * temp_B.pow(1/2.4) - 0.055) * mask_B.float()
